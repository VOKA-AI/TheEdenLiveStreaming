...
export class HlsjsIPFSLoader {
  constructor(config) {
    ...
    this.ipns = config.ipnsAddr
    this.dirName = config.dirName
    this.useIPNS = config.useIPNS;
  ...
  loadInternal() {
    ...
    if (this.useIPNS) {
        getFile(this.ipfs, this.ipns, filename, options, this.debug, this._abortFlag).then(res => {
      ...
      getFile2(this.ipfs, this.dirName, filename, this._abortFlag).then(res => {
        const data = (context.responseType === 'arraybuffer') ? res : buf2str(res)
        stats.loaded = stats.total = data.length
        stats.tload = Math.max(stats.tfirst, performance.now())
        const response = { url: context.url, data: data }
...
async function getFile(ipfs, ipnsAddr, filename, options, debug, abortFlag) {
  var rootHash
  for await (rootHash of ipfs.name.resolve(ipnsAddr)) {}
  rootHash = rootHash.split("/")[2]
...
// 讲m3u8用ipns固定，其中记录ts片段的CID
async function getFile2(ipfs,ipfsDirName, filename, abortFlag) {
  try {
    return await filesRead(ipfsDirName + filename,  ipfs,  abortFlag);
  } catch(ex) {
    throw new Error(`File not found: ${ipfsDirName}/${filename}`)
  }
}
...
async function filesRead (path, ipfs,  abortFlag) {
  const parts = []
  let length = 0, offset = 0
